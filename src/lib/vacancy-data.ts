...
const STL_NEIGHBORHOODS = [
  ...
  { name: "Forest Park Southeast", lat: 38.631, lng: -90.255, vacancyRate: 0.12 },
...
const STREET_NAMES = [
  "N Broadway", "S Broadway", "Natural Bridge Ave", "Dr Martin Luther King Dr",
  "Cass Ave", "Florissant Ave", "N Grand Blvd", "S Grand Blvd",
  "Gravois Ave", "Chippewa St", "Arsenal St", "Manchester Ave",
  "Delmar Blvd", "Page Blvd", "Easton Ave", "Sullivan Ave",
  "Hebert St", "Blair Ave", "Palm St", "Obear Ave",
  "Penrose St", "Marcus Ave", "Margaretta Ave", "Newstead Ave",
  "Prairie Ave", "Benton St", "Montgomery St", "Bacon St",
  "Greer Ave", "Warne Ave", "Evans Ave", "St Louis Ave",
];

const ZIP_CODES = ["63101","63102","63103","63104","63106","63107","63108","63109","63110","63111","63112","63113","63115","63116","63118","63120"];
const ZONING_CODES = ["A","B","C","D","F","G","H","J"];
const COMPLAINT_CATEGORIES = [
  "Vacant Building - Open/Dangerous", "High Weeds/Grass - Vacant",
  "Debris/Refuse Accumulation", "Structural Defect - Residential",
  "Rodent Infestation", "Illegal Dumping", "Property Damage", "Graffiti",
];
...
export function generateVacancyData(): VacantProperty[] {
  ...
  const properties: VacantProperty[] = [];
  let id = 1;
  ...
  for (const hood of STL_NEIGHBORHOODS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const owner = isLRA ? "LRA" as const : isCity ? "CITY" as const : "PRIVATE" as const;

      const condBase = isBuilding
        ? Math.max(1, Math.min(5, Math.round(3 - hood.vacancyRate * 4 + randomFloat(-1, 1.5))))
        : Math.max(1, Math.min(5, Math.round(3.5 - hood.vacancyRate * 2 + randomFloat(-0.5, 1))));

      const lotSqFt = isBuilding ? randomInt(1200, 6000) : randomInt(1500, 15000);
      const taxYearsDelinquent = Math.max(0, isLRA ? randomInt(3, 15) : randomInt(0, 10));
      const complaintsNearby = Math.max(0, Math.round(hood.vacancyRate * 30 + randomFloat(-5, 10)));
      const proximityScore = Math.max(0, Math.min(100, Math.round((1 - hood.vacancyRate) * 80 + randomFloat(-10, 10))));
      const neighborhoodDemand = Math.max(0, Math.min(100, Math.round((1 - hood.vacancyRate * 0.5) * 60 + randomFloat(-15, 25))));
      const boardUpStatus = isBuilding ? (rand() < 0.6 ? "Boarded" : rand() < 0.5 ? "Open" : "Secured") : "N/A";
      const violationCount = Math.max(0, isBuilding ? randomInt(0, 12) : randomInt(0, 4));
      const condemned = isBuilding && condBase <= 1 && rand() < 0.7;
      const assessedValue = isBuilding ? randomInt(2000, 35000) : randomInt(500, 8000);
      ...
      const base = {
        ...
        propertyType: (isBuilding ? "building" : "lot") as "building" | "lot",
        owner,
        ...
        stories,
        recentComplaints,
        vacancyCategory: condemned ? "Definite" : isLRA ? "Definite" : taxYearsDelinquent > 3 ? "Very Likely" : violationCount > 2 ? "Possibly" : "Indeterminant",
      };
