...
export function computeEquityGaps(
  ...
  const desertTracts = foodDeserts.features.filter(
    (f) => (f.properties as FoodDesertProperties).lila,
  );

  const results = desertTracts.map((tract) => {
    const p = tract.properties as FoodDesertProperties;
    const centroid = polygonCentroid(tract.geometry.coordinates as number[][][]);
    ...
    groceryStores.features.forEach((store) => {
      const [sLon, sLat] = store.geometry.coordinates as number[];
      stops.features.forEach((stop) => {
        const [bLon, bLat] = stop.geometry.coordinates as number[];
        ...
        if (distStopToGrocery <= 0.25) {
          ...
          if (stats?.routes.length) {
            stops.features.forEach((tractStop) => {
              const [tLon, tLat] = tractStop.geometry.coordinates as number[];
              const distToTract = haversine(centroid[0], centroid[1], tLat, tLon);
              if (distToTract <= WALK_RADIUS_MILES) {
                const tractStopStats = stopStats[tractStop.properties.stop_id as string];
                if (tractStopStats) {
                  const sharedRoute = tractStopStats.routes.find((r) => stats.routes.includes(r));
                  if (sharedRoute) {
