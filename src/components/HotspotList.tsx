          ...
          return (
            ...
            >
              ...
              <div className="mt-0.5 text-xs text-muted-foreground">
                {type === "volume" ? (
                  <>
                    {item.total.toLocaleString()} complaints &mdash; {item.ratio}x city avg
                    {topCat && <div>Top issue: {topCat[0]} ({topCat[1]})</div>}
                  </>
                ) : (
                  <>
                    {item.avgResolutionDays}d avg resolution &mdash; {item.ratio}x city avg
